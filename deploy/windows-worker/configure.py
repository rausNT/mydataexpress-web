#!/usr/bin/env python3
"""Build the isolated Wine worker config and exact Nginx alias routes."""

from __future__ import annotations

import argparse
import configparser
import json
import os
import re
import tempfile
from pathlib import Path


ALIAS_RE = re.compile(r"^[A-Za-z0-9_]+$")


def windows_path(
    value: str, *, data_root: str = "/var/lib/dataexpress", drive: str = "D:"
) -> str:
    if not value:
        return value
    if re.match(r"^[A-Za-z]:[\\/]", value):
        return value
    if not value.startswith("/"):
        raise ValueError(f"worker filesystem path must be absolute: {value!r}")
    root = data_root.rstrip("/")
    if value != root and not value.startswith(root + "/"):
        raise ValueError(
            f"worker filesystem path is outside {data_root!r}: {value!r}"
        )
    relative = value[len(root) :].lstrip("/")
    return drive + ("\\" + relative.replace("/", "\\") if relative else "\\")


def remote_database(value: str, host: str) -> str:
    if not value:
        raise ValueError("database path is empty")
    if re.match(r"^[^/\\:]+:", value):
        return value
    if not value.startswith("/"):
        raise ValueError(f"database path must be absolute: {value!r}")
    return f"{host}:{value}"


def load_config(path: Path) -> configparser.ConfigParser:
    config = configparser.ConfigParser(interpolation=None, strict=True)
    config.optionxform = str
    with path.open("r", encoding="utf-8-sig") as stream:
        config.read_file(stream)
    if "Server" not in config:
        raise ValueError("source config has no [Server] section")
    return config


def build_worker_config(
    source: configparser.ConfigParser,
    *,
    port: int,
    firebird_host: str,
    data_root: str = "/var/lib/dataexpress",
    database_alias: str | None = None,
) -> configparser.ConfigParser:
    result = configparser.ConfigParser(interpolation=None, strict=True)
    result.optionxform = str
    result["Server"] = dict(source["Server"])
    result["Server"]["Address"] = "127.0.0.1"
    result["Server"]["Port"] = str(port)
    result["Server"]["UseSSL"] = "False"
    result["Server"]["Firebird"] = "5"
    result["Server"]["ShowConnections"] = "False"
    result["Server"]["WindowsWorkerMode"] = "1"

    for section in source.sections():
        if section == "Server":
            continue
        values = dict(source[section])
        if section.lower().startswith("provider:"):
            result[section] = values
            continue
        if database_alias is not None and section != database_alias:
            continue
        if not ALIAS_RE.fullmatch(section):
            raise ValueError(f"database alias is not routable: {section!r}")
        values["Database"] = remote_database(
            values.get("Database", ""), firebird_host
        )
        if values.get("Templates"):
            values["Templates"] = windows_path(
                values["Templates"], data_root=data_root
            )
        result[section] = values
    return result


def database_aliases(source: configparser.ConfigParser) -> list[str]:
    aliases = [
        section
        for section in source.sections()
        if section != "Server" and not section.lower().startswith("provider:")
    ]
    seen: set[str] = set()
    for alias in aliases:
        if not ALIAS_RE.fullmatch(alias):
            raise ValueError(f"database alias is not routable: {alias!r}")
        folded = alias.casefold()
        if folded in seen:
            raise ValueError(f"database aliases differ only by case: {alias!r}")
        seen.add(folded)
    return sorted(aliases, key=str.casefold)


def plan_worker_instances(
    source: configparser.ConfigParser, base_port: int
) -> list[dict[str, object]]:
    aliases = database_aliases(source)
    if base_port + len(aliases) - 1 > 65535:
        raise ValueError("not enough TCP ports for database worker instances")
    return [
        {"alias": alias, "instance": alias.casefold(), "port": base_port + index}
        for index, alias in enumerate(aliases)
    ]


def build_routes(
    source: configparser.ConfigParser, upstream: str | dict[str, str]
) -> str:
    blocks = [
        "# Generated by DataExpress Windows worker. Do not edit by hand.",
    ]
    for section in database_aliases(source):
        alias = section.lower()
        target = upstream[section] if isinstance(upstream, dict) else upstream
        blocks.extend(
            [
                f"location ^~ /{alias}/ {{",
                "    limit_req zone=dx_requests burst=80 nodelay;",
                "    limit_req zone=dx_logins burst=10 nodelay;",
                f"    proxy_pass http://{target};",
                "    proxy_http_version 1.1;",
                "    proxy_set_header Host $host;",
                "    proxy_set_header X-Real-IP $remote_addr;",
                "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
                "    proxy_set_header X-Forwarded-Proto $scheme;",
                "    proxy_set_header X-Request-ID $request_id;",
                "}",
            ]
        )
    return "\n".join(blocks) + "\n"


def configure_instances(args: argparse.Namespace, source: configparser.ConfigParser) -> None:
    instances = plan_worker_instances(source, args.base_port)
    upstreams: dict[str, str] = {}
    for instance in instances:
        alias = str(instance["alias"])
        instance_id = str(instance["instance"])
        port = int(instance["port"])
        worker = build_worker_config(
            source,
            port=port,
            firebird_host=args.firebird_host,
            data_root=args.data_root,
            database_alias=alias,
        )
        atomic_write(
            args.instances_root / instance_id / "dxwebsrv.cfg",
            serialize_config(worker),
            0o640,
        )
        upstreams[alias] = f"127.0.0.1:{port}"
    atomic_write(args.routes, build_routes(source, upstreams), 0o644)
    atomic_write(
        args.manifest,
        json.dumps({"schemaVersion": 1, "instances": instances}, ensure_ascii=False, indent=2)
        + "\n",
        0o640,
    )


def atomic_write(path: Path, data: str, mode: int) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    handle, temporary_name = tempfile.mkstemp(
        prefix=f".{path.name}.", dir=path.parent
    )
    temporary = Path(temporary_name)
    try:
        with os.fdopen(handle, "w", encoding="utf-8", newline="\n") as stream:
            stream.write(data)
            stream.flush()
            os.fsync(stream.fileno())
        os.chmod(temporary, mode)
        os.replace(temporary, path)
    finally:
        temporary.unlink(missing_ok=True)


def serialize_config(config: configparser.ConfigParser) -> str:
    from io import StringIO

    output = StringIO()
    config.write(output, space_around_delimiters=False)
    return output.getvalue()


def configure(args: argparse.Namespace) -> None:
    source = load_config(args.source)
    if args.instances_root is not None:
        configure_instances(args, source)
        return
    worker = build_worker_config(
        source,
        port=args.port,
        firebird_host=args.firebird_host,
        data_root=args.data_root,
    )
    routes = build_routes(source, args.upstream)
    atomic_write(args.worker_config, serialize_config(worker), 0o640)
    atomic_write(args.routes, routes, 0o644)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source", type=Path, required=True)
    parser.add_argument("--worker-config", type=Path)
    parser.add_argument("--instances-root", type=Path)
    parser.add_argument("--manifest", type=Path)
    parser.add_argument("--routes", type=Path, required=True)
    parser.add_argument("--port", type=int, default=8180)
    parser.add_argument("--base-port", type=int, default=8180)
    parser.add_argument("--upstream", default="127.0.0.1:8180")
    parser.add_argument("--firebird-host", default="127.0.0.1")
    parser.add_argument("--data-root", default="/var/lib/dataexpress")
    args = parser.parse_args()
    if not 1024 <= args.port <= 65535:
        parser.error("--port must be between 1024 and 65535")
    if not 1024 <= args.base_port <= 65535:
        parser.error("--base-port must be between 1024 and 65535")
    if not re.fullmatch(r"127\.0\.0\.1:\d{2,5}", args.upstream):
        parser.error("--upstream must be a loopback IPv4 host and port")
    if args.firebird_host not in {"127.0.0.1", "localhost"}:
        parser.error("--firebird-host must be loopback")
    if not args.data_root.startswith("/"):
        parser.error("--data-root must be absolute")
    if args.instances_root is not None:
        if args.manifest is None:
            parser.error("--manifest is required with --instances-root")
    elif args.worker_config is None:
        parser.error("--worker-config or --instances-root is required")
    return args


if __name__ == "__main__":
    configure(parse_args())
