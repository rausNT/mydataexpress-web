...
- изолированные по базе `.wepas`: предпочтительно модуль хранится внутри `.DXDB`,
  а внешний адаптер назначается только конкретному подключению через
  `extensions/<имя-подключения>/`;
- проверенные по хешу официальные web-версии `DX_PLUS 1.71`–`1.8.1`, которые
  можно явно назначить проверенной базе по GUID действий и именам функций, из
