...
настольной зависимостью получают provider-границу. Для четырёх из них handler
генерируется автоматически: старого `HTTP_GET` и трёх stateful-вызовов DaData
(`DA_FIRM_GET`, `DA_BANK_GET`, `DA_ADDR_GET`). Оставшиеся две операции
конвертации Word/Excel требуют document-provider. Два HTTP mappings остаются работоспособным Pascal Script,
