...
Автоматический fallback не переопределяет написанный автором `.wepas`.
Поскольку DataExpress не контролирует версии расширений, web-модуль должен
храниться в самой `.DXDB` либо быть явно назначен одному подключению в
`extensions/<имя-подключения>/`. Общего автоматического каталога для всех баз
нет: одинаковое имя или GUID ещё не доказывают совместимость разных копий
расширения. Сопоставление внутри конкретной базы выполняется по метаданным
`Name`/`Id`. Проверка консервативна: прямые процессы, DLL, COM/OLE, сетевые и
...
Текущий установщик создаёт отдельный systemd-экземпляр, Wine-prefix, каталог
runtime и loopback-порт для каждого подключения из `dxwebsrv.cfg`. В экземпляр
копируется только `extensions/<имя-подключения>/`; контрольный manifest содержит
пути, размеры и SHA-256 фактически размещённых `.epas`, `.wepas` и DLL. Удалённые
из конфигурации экземпляры отключаются, но их каталоги автоматически не
стираются. Это исключает случайное смешивание несовместимых версий расширений и
оставляет администратору возможность восстановить прежний комплект.
